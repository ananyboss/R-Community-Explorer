data <- read.csv(paste("dailies/",toString(format(Sys.Date()-1, "%Y")),"/",toString(format(Sys.Date()-1, "%b")),"/",toString(format(Sys.Date()-1, "%Y")),"-",toString(format(Sys.Date()-1, "%b")),"-",toString(format(Sys.Date()-1, "%d")),".csv",sep=""))
arr<-data$is_retweet
cnt_tweet <- 0
cnt_re <- 0
for (ele in arr){
